				...
				if(cnode == NULL) {
					...
					do { // seq must have length > 0
						...
						if( cnode->del ) {
							...
							SAVE_STATE_WITH_THREAD(rp_lookup_superSymbol, failureThreads, currThreadId, cnode, del);
...
//				SAVE_STATE_WITH_THRESHOLD_AND_THREAD(rp_lookup_superSymbol, &(cnode->version), cnode->version+1, cnode->delThread)
...
//					LinkedStack<CoroutineState *> *restartFromNewNode = VAR(restartFrom);
			...
			// source function symbol
					...
					if( cnode->del ) {
						...
						SAVE_STATE_WITH_THREAD(rp_lookup_funcSymbol, failureThreads, currThreadId, cnode, del);
						failureThreads->delFlag = 1;
...
//                assert((subterm == NULL) == VAR(inp)->newVar);
		...
		//printf("from: \n%s", triestrbuf);
				...
				if(VAR(varInp)->repeatedVar != 0) {
					subNew = VAR(sub)->push(VAR(varInp)->symbol, retterm, subRegion);
				} else {
					subNew = VAR(sub);
				}
			} else {
		...
		printf("subterm: %s\n", triestrbuf);
#endif
			rp_lookup_checkTermSize:
				nInput = VAR(n) + VAR(subterm)->termSize() - 1;
				CHECK_TERM_SIZE(nInput, rp_lookup_checkTermSize);
...
//rp_branch:
	if( VAR(numberOfPlaceholders) == 0 ) {
		...
		FUNC_TAIL_CALL(branchTryTargetNodes, (VAR(curr)->funcChild));
	}
END
#undef FUN
#define FUN branchTryTargetNodes
    PARAMS(
            Trie *, curr)
    RETURNS(Trie *, Term *, int)
BEGIN
		...
		if(VAR(curr)->del) {
			SAVE_STATE_WITH_THREAD(rp_branchTryTargetNodes_checkTermSize_del, failureThreads, currThreadId, VAR(curr), del);
			failureThreads->delFlag = 1;
			if(VAR(curr)->sibling != NULL) {
				FUNC_TAIL_CALL(branchTryTargetNodes, (VAR(curr)->sibling));
			} else {
				// failure condition 4
				SAVE_STATE_WITH_THREAD(rp4, failureThreads, currThreadId, VAR(curr), sibling);
				failureThreads->delFlag = 1;
				RESTORE_STATE;
			}
		rp_branchTryTargetNodes_checkTermSize_del:
			s = VAR(curr)->key;
			arity = IS_FUNC_SYMBOL(s) ? (int) (s & FUNC_ID_ARITY_MASK) : 0;
			nInput = VAR_F1(branch, n) + VAR_F1(branch, numberOfPlaceholders) + arity;
			CHECK_TERM_SIZE(nInput, rp_branchTryTargetNodes_checkTermSize);
		} else {
		rp_branchTryTargetNodes_checkTermSize:
			s = VAR(curr)->key;
			arity = IS_FUNC_SYMBOL(s) ? (int) (s & FUNC_ID_ARITY_MASK) : 0;
			nInput = VAR_F1(branch, n) + VAR_F1(branch, numberOfPlaceholders) + arity;
			CHECK_TERM_SIZE(nInput, rp_branchTryTargetNodes_checkTermSize);

			// before and following a save_state whose resumption point is a destructive update of the current activation record,
			// there may not be any more save_state actions before a exit action such as return, tail_call, or restore_state
			if(VAR(curr)->sibling != NULL) {
				SAVE_STATE(rp4_replace);
			} else {
				SAVE_STATE_WITH_THREAD(rp4_replace, failureThreads, currThreadId, VAR(curr), sibling);
				failureThreads->delFlag = 1;
			}
		}

        FUNC_TAIL_CALL(branch,
        		(
        				VAR_F1(branch, n) + 1,
        				VAR_F1(branch, start),
        				VAR(curr),
        				VAR_F1(branch, numberOfPlaceholders) + arity - 1
        		)
        );
	...
	rp4:
		FUNC_TAIL_CALL(branchTryTargetNodes, (VAR(curr)->sibling));
	rp4_replace:
		VAR(curr) = VAR(curr)->sibling;
		goto ENT_POINT(branchTryTargetNodes);
